...
Standard Hashing Algorithm 3 (SHA-3)'s Kotlin implementation compiled to Javascript for NPM.

Parent Source: [KeccakKotlin](https://github.com/ronhombre/KeccakKotlin)
...
## Documentation
Visit [keccak.hombre.asia](https://keccak.hombre.asia). All **common** methods and classes are available in JS.

For more details, view the [NIST FIPS 202](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.202.pdf) document.
...
function test224() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.SHA3_224, Buffer.from(""))
...
function test256() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.SHA3_256, Buffer.from(""))
...
function test384() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.SHA3_384, Buffer.from(""))
...
function test512() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.SHA3_512, Buffer.from(""))
...
function testrawshake128() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.RAWSHAKE_128, Buffer.from(""))
...
function testrawshake256() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.RAWSHAKE_256, Buffer.from(""))
...
function testshake128() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.SHAKE_128, Buffer.from(""))
...
function testshake256() {
    ...
    let hash = KeccakHash.generate(KeccakParameter.SHAKE_256, Buffer.from(""))
